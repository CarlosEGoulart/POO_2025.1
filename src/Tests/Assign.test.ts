...
test("Test Assign Artist to Art", () => {
    ...
    const assigned = artController.assignArtistToArt(createdArt.getId(), createdArtist.getId());

    expect(assigned).toBe(true);
});

test("Test Assign Art to Exhibition", () => {
    const artController = new ArtController(db);
    ...
    const assigned = exhibitionController.addArtToExhibition(createdExhibition.getId(), createdArt.getId());
