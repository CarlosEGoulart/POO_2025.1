...
export default class ExhibitionView {
  ...
  private createExhibition(): void {
    ...
    const description = readlineSync.question(
      "Digite a descrição da exposição: "
    );

    const newExhibition = this.exhibitionController.createExhibition(
      name,
      description
    );
    if (newExhibition) {
      console.log(
        `Exposição "${newExhibition.getName()}" criada com ID: ${newExhibition.getId()}.`
      );
    } else {
  ...
  private viewExhibitionDetails(): void {
    ...
    if (exhibitionToView) {
      ...
      const arts = this.exhibitionController.getExhibitionArts(
        exhibitionToView.getId()
      );
  ...
  private updateExhibition(): void {
    ...
    if (searchOption === "ID") {
      exhibitionIdToUpdate = readlineSync.questionInt(
        "Digite o ID da exposição para atualizar: "
      );
      exhibitionToUpdate =
        this.exhibitionController.getExhibition(exhibitionIdToUpdate);
    } else if (searchOption === "NOME") {
      const exhibitionName = readlineSync.question(
        "Digite o nome da exposição para atualizar: "
      );
      exhibitionToUpdate =
        this.exhibitionController.getExhibitionByName(exhibitionName);
    ...
    if (exhibitionToUpdate) {
      ...
      const newArtWorksInput = readlineSync.question(
        `Novas obras (IDs separados por vírgula, exemplo: 1,2,3,4): `
      );
      ...
      if (exhibitionIdToUpdate !== undefined) {
        const updated = this.exhibitionController.updateExhibition(
          newIdExhibition,
          newName,
          newDescription,
          artWorksArray
        ...
        console.log(
          "Erro interno: ID da exposição não disponível para atualização."
        );
      }
    } else {
      console.log("Exposição não encontrada.");
  ...
  private deleteExhibition(): void {
    ...
    if (searchOption === "ID") {
      exhibitionToDeleteId = readlineSync.questionInt(
        "Digite o ID da exposição para deletar: "
      );
    } else if (searchOption === "NOME") {
      exhibitionToDeleteName = readlineSync.question(
        "Digite o nome da exposição para deletar: "
      );
      const exhibitionToDelete = this.exhibitionController.getExhibitionByName(
        exhibitionToDeleteName
      );
      if (exhibitionToDelete) {
        exhibitionToDeleteId = exhibitionToDelete.getId();
      } else {
        console.log(
          `Exposição com nome "${exhibitionToDeleteName}" não encontrada.`
        );
        return;
      }
    } else {
    ...
    if (exhibitionToDeleteId !== undefined) {
      const deleted =
        this.exhibitionController.deleteExhibition(exhibitionToDeleteId);
      if (deleted) {
  ...
  private addArtToExhibition(): void {
    console.log(
      "Adicionar obra de arte à exposição: Ainda em desenvolvimento."
    );
  }
